...
| Sesja crasha | reporter po podpięciu natychmiast przejmuje także kontynuowaną sesję z dysku; ponowne `configure` zatrzymuje poprzedni handler i timer przed instalacją nowych | [x] |
| Nazwy wątków | tylko nazwy o pewnym pochodzeniu (np. main thread dla hanga). Prywatny layout `dispatch_qaddr` nie jest interpretowany jako C string; niepoprawne UTF-8 i znaki kontrolne zostają brakiem | [x] |
